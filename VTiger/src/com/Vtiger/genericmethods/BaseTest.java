...
public class BaseTest implements ITconstant {
	...
	{
		try {
			Excel.createExcel(REPORT_PATH, REPORT_SHEET);
	...
	public static void afterM(ITestResult result) throws Throwable {
		...
		if (status == 1) {
			...
			Excel.setData(testCase, "pass", r, c, REPORT_SHEET, REPORT_PATH);
			r++;
			c = 0;
		} else {
			String path = IMGPATH + testCase + ".png";
			CaptureScreenShot.getScreenShot(driver, path);
			Reporter.log(testCase + "is fail", true);
			Excel.setData(testCase, "fail", r, c, REPORT_SHEET, REPORT_PATH);
			r++;
			c = 0;
